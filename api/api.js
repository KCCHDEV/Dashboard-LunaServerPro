...
app.get("/api/setbooster", async(req, res) => {
    let settings = JSON.parse(fs.readFileSync("./settings.json").toString());
    if(req.query.key !== settings.api.client.api.code) return res.send("Invalid API Key.");
    if(!req.query.id) return res.send("No user ID.");
    if (!(await db.get("users-" + req.query.id))) return res.send("User never signed up.");
    if(req.query.boosted == "true") {
    await db.set("package-" + req.query.id, "booster");
    return res.send("Set user "+req.query.id+" to booster!");
    }else{
    await db.set("package-"+req.query.id, "default");
    return res.send("Set user "+req.query.id+" to default!");
    }
})
